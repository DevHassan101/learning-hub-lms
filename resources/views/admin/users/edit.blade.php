...
<x-app-layout>
    ...
    <div class="w-[80%] mx-auto">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-[#3bcbbe]">Edit User</h1>
                <p class="text-md font-medium text-gray-600 mt-0.5">Update user account information</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#3bcbbe] overflow-hidden shadow-md rounded-2xl">
            ...
            <div class="p-8">
                <form action="{{ route('users.update', ['user' => $user]) }}" method="post" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" id="name" placeholder="Enter full name" name="name" value="{{ $user->name }}"
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email" placeholder="Enter email address" value="{{ $user->email }}"
                            class="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 h-14 outline-none focus:ring-2 focus:ring-[#3bcbbe] focus:border-[#3bcbbe] focus:bg-white transition-all text-md placeholder:text-gray-400 shadow-sm hover:border-[#3bcbbe]/50">
                        @error('email')
                            <span class="text-red-600 text-xs font-medium mt-1.5 flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
