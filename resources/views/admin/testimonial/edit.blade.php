...
<x-app-layout>
    ...
    <div class="w-[80%] mx-auto">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-[#3bcbbe]">Edit Testimonial</h1>
                <p class="text-md font-medium text-gray-600 mt-0.5">Update testimonial information</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#3bcbbe] overflow-hidden shadow-md rounded-2xl">
            ...
            <div class="p-8">
                <form action="{{ route('testimonial.update', ['testimonial' => $testimonial]) }}" method="post" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" id="name" placeholder="Enter student name" name="name" value="{{ old('name', $testimonial->name) }}"
                    ...
                    <div>
                        ...
                        <textarea name="message" id="message" rows="8" placeholder="Enter testimonial message"
                            class="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#3bcbbe] focus:border-[#3bcbbe] focus:bg-white transition-all text-md placeholder:text-gray-400 shadow-sm hover:border-[#3bcbbe]/50 resize-none">{{ old('message', $testimonial->message) }}</textarea>
                        @error('message')
                            <span class="text-red-600 text-xs font-medium mt-1.5 flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
