...
<x-app-layout>
    ...
    <div class="w-[80%] mx-auto">
        ...
        <div class="mb-6 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-bold text-[#3bcbbe]">Edit Flash Card</h1>
                <p class="text-md font-medium text-gray-600 mt-0.5">Update flash card information</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-[#3bcbbe] overflow-hidden shadow-md rounded-2xl">
            ...
            <div class="p-8">
                <form action="{{ route('flash-card.update', ['flash_card' => $flashCard]) }}" method="post" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="text" id="title" placeholder="Enter flash card title" name="title" value="{{ old('title', $flashCard->title) }}"
                    ...
                    <div>
                        ...
                        <select name="course_id" id="course_id"
                            ...
                            @foreach ($courses as $course)
                                <option value="{{$course->id}}" @selected($course->id == $flashCard->course_id)>{{$course->title}}</option>
                            @endforeach
                        </select>
                        @error('course_id')
                            <span class="text-red-600 text-xs font-medium mt-1.5 flex items-center gap-1">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="8" placeholder="Enter flash card description"
                            class="w-full bg-gray-50 border border-gray-300 rounded-xl px-4 py-3 outline-none focus:ring-2 focus:ring-[#3bcbbe] focus:border-[#3bcbbe] focus:bg-white transition-all text-md placeholder:text-gray-400 shadow-sm hover:border-[#3bcbbe]/50 resize-none">{{ old('description', $flashCard->description) }}</textarea>
                        @error('description')
                            <span class="text-red-600 text-xs font-medium mt-1.5 flex items-center gap-1">
                                <svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20">
                                    <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
